...
@router.get("/duration/min", response_description="Get trip duration minimum in minutes")
def get_trip_duration_min(request: Request):
    mongo_client = MongoClient(MONGODB_URL, connect=False)
    database = mongo_client.mongodb_client[DB_NAME]

    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_trip_duration = {"$project": {"duration": {"$subtract":["$finish", "$start"]}}}
    stage_trip_duration_min = {"$group": { "_id": None, "min_duration": {"$min": "$duration"}}}
    pipeline = [
...
@router.get("/duration/max", response_description="Get trip duration maximum in minutes")
def get_trip_duration_max(request: Request):
    ...
    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_trip_duration = {"$project": {"duration": {"$subtract":["$finish", "$start"]}}}
    stage_trip_duration_max = {"$group": { "_id": None, "max_duration": {"$max": "$duration"}}}
    pipeline = [
        stage_match_terminated_status,
        stage_trip_duration,
...
@router.get("/duration/avg", response_description="Get trip duration average in minutes")
def get_trip_duration_avg(request: Request):
    ...
    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_trip_duration = {"$project": {"duration": {"$subtract":["$finish", "$start"]}}}
    stage_trip_duration_avg = {"$group": { "_id": None, "avg_duration": {"$avg": "$duration"}}}
    pipeline = [
        stage_match_terminated_status,
        stage_trip_duration,

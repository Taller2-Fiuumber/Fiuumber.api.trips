...
@router.get("/duration/months/range", response_description="Count new trips last n months")
def count_trips_new_count_last_n_months_range(amount: int, request: Request):
    ...
    stage_trip_duration = {
        "$project": {"duration": { "$divide": [{"$subtract": ["$finish", "$start"]}, 60000]} }
    }
    stage_trip_count = {"$group": {
        "_id": "$duration",
        "count": { "$sum": 1 }
    }}
    pipeline = [stage_match_terminated_status, stage_match_last_n_month, stage_trip_duration, stage_trip_count]
...
@router.get("/duration/months-and-months/range", response_description="Count new trips last n months")
def count_trips_new_count_last_n_months_range(years: int, months: int, request: Request):
    mongo_client = MongoClient(MONGODB_URL, connect=False)
    database = mongo_client.mongodb_client[DB_NAME]

    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_match_last_n_years_month = {"$match": {"start": {"$gte" : datetime.today() - relativedelta(years=+years, months=+months)}}}
    stage_trip_duration = {
        "$project": {"duration": { "$divide": [{"$subtract": ["$finish", "$start"]}, 60000]} }
    }
    stage_trip_count = {"$group": {
    ...
    pipeline = [stage_match_terminated_status, stage_match_last_n_years_month, stage_trip_duration, stage_trip_count]
...
@router.get("/duration/years/range", response_description="Count new trips last n years")
def count_trips_duration_last_n_years_range(amount: int, request: Request):
    mongo_client = MongoClient(MONGODB_URL, connect=False)
    database = mongo_client.mongodb_client[DB_NAME]

    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_match_last_n_years = {"$match": {"start": {"$gte" : datetime.today() - relativedelta(years=+amount)}}}
    stage_trip_duration = {
        "$project": {"duration": { "$divide": [{"$subtract": ["$finish", "$start"]}, 60000]} }
    }
    stage_trip_count = {"$group": {
    ...
    pipeline = [stage_match_terminated_status, stage_match_last_n_years, stage_trip_duration, stage_trip_count]
...
def count_trips_duration_last_n_days_range(amount: int, request: Request):
    ...
    pipeline = [stage_match_terminated_status, stage_match_last_n_days, stage_trip_duration, stage_trip_count]

...
def find_fare_avg(request: Request):
    ...
    stage_trip_duration_avg = {"$group": { "_id": None, "avg_final_price": {"$avg": "$finalPrice"}}}
    pipeline = [
...
def find_fare_min(request: Request):
    ...
    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_trip_duration_min = {"$group": { "_id": None, "min_final_price": {"$min": "$finalPrice"}}}
    pipeline = [
        stage_match_terminated_status,
...
def find_fare_min(request: Request):
    ...
    stage_match_terminated_status = {"$match": {"status": "TERMINATED"}}
    stage_trip_duration_max = {"$group": { "_id": None, "max_final_price": {"$max": "$finalPrice"}}}
    pipeline = [
        stage_match_terminated_status,
